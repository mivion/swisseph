...
swisseph.calc = function (options, callback) {
	...
	options.date.julian = swisseph.swe_julday (options.date.year, options.date.month, options.date.day, options.date.hours + (options.date.seconds / 60 + options.date.minutes) / 60, swisseph.SE_GREG_CAL);
	options.date.delta = swisseph.swe_deltat (options.date.julian).delta;
	options.date.universal = {
		julian: options.date.julian - options.date.delta
	};

	utils.merge (options.date.universal, swisseph.swe_revjul (options.date.universal.julian, swisseph.SE_GREG_CAL));

	utils.merge (options.body, swisseph.swe_get_planet_name (parseInt (options.body.id)));
	...
	if (options.observer.geographic.height > 0) {
	...
	options.body.position = swisseph.swe_calc_ut (options.date.universal.julian, parseInt (options.body.id), flags);
	options.body.position.longitude = {
		decimalDegree: options.body.position.longitude
	};
	options.body.position.latitude = {
		decimalDegree: options.body.position.latitude
	};
