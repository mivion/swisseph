...
swisseph.calc = function (options, callback) {
	...
	if (options.date.gregorian.terrestrial.changed) {
		date = options.date.gregorian.terrestrial.value;
		options.date.julian.terrestrial.value = swisseph.swe_julday (date.year, date.month, date.day, date.hours + (date.seconds / 60 + date.minutes) / 60, swisseph.SE_GREG_CAL);
		options.date.julian.delta.value = swisseph.swe_deltat (options.date.julian.terrestrial.value).delta;
		options.date.julian.universal.value = options.date.julian.terrestrial.value - options.date.julian.delta.value;
		options.date.gregorian.universal.value = options.date.gregorian.universal.value || {};
		utils.merge (options.date.gregorian.universal.value, swisseph.swe_revjul (options.date.julian.universal.value, swisseph.SE_GREG_CAL));
	} else if (options.date.gregorian.universal.changed) {
		date = options.date.gregorian.universal.value;
		options.date.julian.universal.value = swisseph.swe_julday (date.year, date.month, date.day, date.hours + (date.seconds / 60 + date.minutes) / 60, swisseph.SE_GREG_CAL);
		options.date.julian.delta.value = swisseph.swe_deltat (options.date.julian.universal.value).delta;
		options.date.julian.terrestrial.value = options.date.julian.universal.value + options.date.julian.delta.value;
		options.date.gregorian.terrestrial.value = options.date.gregorian.terrestrial.value || {};
		utils.merge (options.date.gregorian.terrestrial.value, swisseph.swe_revjul (options.date.julian.terrestrial.value, swisseph.SE_GREG_CAL));
	} else if (options.date.julian.terrestrial.changed) {
		options.date.julian.delta.value = swisseph.swe_deltat (options.date.julian.terrestrial.value).delta;
		options.date.julian.universal.value = options.date.julian.terrestrial.value - options.date.julian.delta.value;
		options.date.gregorian.universal.value = options.date.gregorian.universal.value || {};
		options.date.gregorian.terrestrial.value = options.date.gregorian.terrestrial.value || {};
		utils.merge (options.date.gregorian.terrestrial.value, swisseph.swe_revjul (options.date.julian.terrestrial.value, swisseph.SE_GREG_CAL));
		utils.merge (options.date.gregorian.universal.value, swisseph.swe_revjul (options.date.julian.universal.value, swisseph.SE_GREG_CAL));
	} else if (options.date.julian.universal.changed) {
		options.date.julian.delta.value = swisseph.swe_deltat (options.date.julian.universal.value).delta;
		options.date.julian.terrestrial.value = options.date.julian.universal.value + options.date.julian.delta.value;
		options.date.gregorian.universal.value = options.date.gregorian.universal.value || {};
		options.date.gregorian.terrestrial.value = options.date.gregorian.terrestrial.value || {};
	...
	options.body.name.value = swisseph.swe_get_planet_name (parseInt (options.body.id.value)).name;

	options.observer.ephemeris.value = options.observer.ephemeris.value || 'moshier';

	if (swisseph.ephemeris [options.observer.ephemeris.value].length) {
		flags |= swisseph.SEFLG_JPLEPH;
		swisseph.swe_set_jpl_file (swisseph.ephemeris [options.observer.ephemeris.value]);
	} else {
		flags |= swisseph.ephemeris [options.observer.ephemeris.value];
	}

	if (
		options.observer.geographic.longitude.value != 0 ||
		options.observer.geographic.latitude.value != 0 ||
		options.observer.geographic.height.value != 0
	) {
		swisseph.swe_set_topo (
			options.observer.geographic.longitude.value,
			options.observer.geographic.latitude.value,
			options.observer.geographic.height.value
		);
		flags |= swisseph.SEFLG_TOPOCTR;
	}

	var position = swisseph.swe_calc_ut (options.date.julian.universal.value, parseInt (options.body.id.value), flags);
	options.body.position.longitude.decimalDegree.value = position.longitude;
	options.body.position.longitudeSpeed.value = position.longitudeSpeed;
	options.body.position.latitude.decimalDegree.value = position.latitude;
	options.body.position.latitudeSpeed.value = position.latitudeSpeed;
	options.body.position.distance.value = position.distance;
	options.body.position.distanceSpeed.value = position.distanceSpeed;
